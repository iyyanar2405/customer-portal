...
@Component({
  ...
  imports: [CommonModule, TranslocoModule, GridComponent],
  ...
  implements OnDestroy
{
  ...
  isPreferenceInitialized = this.preferenceDataLoaded.pipe(
    filter((value) => value),
    tap(() => {
      this.settingsMembersStoreService.loadSettingsMembersList();
    }),
  );
  ...
  onGridConfigChanged(gridConfig: GridConfig): void {
    this.settingsMembersStoreService.updateMembersGridConfig(gridConfig);
  }
  ...
  ngOnDestroy(): void {
    this.settingsMembersStoreService.resetMembersListState();
  }
